...
function updateMetadata(song) {
  (player_song.innerHTML = `<marquee>${song.title} - ${song.artist}</marquee>`),
...
(xmlhttp.onreadystatechange = function () {
  if (xmlhttp.readyState == XMLHttpRequest.DONE && 200 == xmlhttp.status) {
      ...
      switch (((section.className = "back_img"), i)) {
        ...
        case Math.round(songs.length / 2) - 1:
          ...
          arrow.classList.add("arrow"),
            ...
            (img_section.src = "img/bottom.png");
          break;
        default:
    ...
    for (var i = 0; i < songs.length; i++) {
      ...
      var songText =
        song.length > 10 ? `<marquee>${song}</marquee>` : `${song}`;
      (inner.innerHTML += `<div class="cont_song"><h4>${songText}</h4><p>${artist}</p></div>`),
