...
function eje(index) {
    ...
    if (index != currentSong)
        ...
        isPlaying
            ? (audio.pause(), songsDivs[currentSong].classList.remove("current"), (buttonsDivs[currentSong].src = "img/play_button.png"), player.classList.remove("current"), (button_player_artist.src = "img/play_button.png"))
            : (audio.play().catch((_) => {}), songsDivs[currentSong].classList.add("current"), (buttonsDivs[currentSong].src = "img/pause_button.png"), player.classList.add("current"), (button_player_artist.src = "img/pause_button.png"));
    }
}
function nextSong() {
...
function updateMetadata(song) {
    (player_song.innerHTML = song.title),
        (player_artist.innerHTML = song.artist),
        "mediaSession" in navigator && (navigator.mediaSession.metadata = new MediaMetadata({ title: song.title, artist: song.artist, album: `${song.title} - ${song.artist}` }));
}
...
function amountscrolled() {
    ...
    for (let i = 0; i < treeImgs.length; i++) {
        ...
        element.style.filter = ` sepia(${0.95 * pctScrolled}%) contrast(100%) saturate(${10 + 7 * pctScrolled}%) hue-rotate(0deg)`;
    }
}
(xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == XMLHttpRequest.DONE && 200 == xmlhttp.status) {
        songs = xmlhttp.responseText.match(regex) || [];
        for (var random = Math.floor(Math.random() * songs.length), i = 0; i < Math.round(songs.length / 2); i++) {
            ...
            switch (((section.className = "back_img"), i)) {
                ...
                case Math.round(songs.length / 2) - 1:
                    ...
                    (img_grass.src = "img/grass.png"), (img_grass.id = "grass"), section.appendChild(img_grass);
                    let arrow = document.createElement("div");
                    arrow.classList.add("arrow"), section.appendChild(arrow), (section.id = "grass_section"), (img_section.src = "img/bottom.png");
                    break;
                default:
                    img_section.src = "img/middle.png";
            }
            img_section.classList.add("tree"), section.appendChild(img_section), background.appendChild(section);
        }
        for (var i = 0; i < songs.length; i++) {
            ...
            var withoutSpaces,
                withoutMp3,
                withoutFolder,
                splitted = decodeURI(urlSong).replace(".mp3", "").replace("music/", "").split("-", 2),
                song = splitted[0],
                artist = splitted[1];
            null != song && (song = song.trim()), null != artist && (artist = artist.trim());
            ...
            i == random
                ? ((currentSong = i),
                  ...
                  /Firefox/i.test(navigator.userAgent)
                      ? (audio.play().catch((_) => {}),
                        (par.className += " current"),
                        (player.className += " current"),
                        (button_player_artist.src = "img/pause_button.png"),
                        (inner.innerHTML = `<img class="action-btn" onclick="eje(${i});" width="80" src="img/pause_button.png" alt="" />`))
                ...
                (inner.innerHTML += `<div><h4>${song}</h4><p>${artist}</p></div>`),
    ...
    window.addEventListener("load", function (event) {
        setTimeout(() => {
            ...
            setTimeout(() => {
                (document.getElementById("overlay").style.opacity = "0"), (document.getElementById("overlay").style.display = "none");
            }, 4200);
    });
